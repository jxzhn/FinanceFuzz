...
from z3 import BitVecVal, BitVecRef, UDiv, URem, SRem, UGT, ULT, UGT, ULT, If, LShR, simplify, is_expr, is_bv_value
...
def print_stack(stack: list[list[BitVecRef] | Literal[False]]) -> None:
    string = '['
...
def print_memory(memory: dict[int, list[BitVecRef] | Literal[False]]) -> None:
...
def print_storage(storage: dict[HexAddress | None, dict[str, list[BitVecRef] | Literal[False]]]) -> None:
...
class TaintRecord:
    def __init__(self, input: dict[str, list[BitVecRef]] | Literal[False] = {}, value: list[BitVecRef] | Literal[False] = False, output: list[BitVecRef] | Literal[False] = False, address: HexAddress | None = None) -> None:
        ''' Builds a taint record '''
        # Execution environment
        self.input: dict[str, list[BitVecRef]] | Literal[False] = input
        self.value: list[BitVecRef] | Literal[False] = value
        self.output: list[BitVecRef] | Literal[False] = output
        self.address: HexAddress | None = address
        # Machine state
        self.stack: list[list[BitVecRef] | Literal[False]] = []
        self.memory: dict[int, list[BitVecRef] | Literal[False]] = {}
...
class SymbolicTaintAnalyzer:
    ...
    def __init__(self) -> None:
        # Machine state
        self.callstack: list[list[TaintRecord]] = []
        # World state
        self.storage: dict[HexAddress | None, dict[str, list[BitVecRef] | Literal[False]]] = {}

    def propagate_taint(self, instruction: TracedInstruction, address: HexAddress | None) -> None:
    ...
    def introduce_taint(self, taint: BitVecRef, instruction: TracedInstruction) -> None:
        if not instruction['error'] and instruction['depth'] - 1 < len(self.callstack):
            ...
            for i in range(1, mutator[1] + 1):
                if not records[-1].stack[-i]:
                    ...
                    record = cast(list[BitVecRef], records[-1].stack[-i])
            ...
            if instruction['op'] in ['CALL', 'CALLCODE', 'DELEGATECALL', 'STATICCALL']:
                ...
                cast(list[BitVecRef], records[-1].memory[convert_stack_value_to_int(instruction['stack'][-1])]).append(taint)

    def check_taint(self, instruction: TracedInstruction, source: BitVecRef | None = None) -> TaintRecord | None:
        if not instruction['error'] and instruction['depth'] - 1 < len(self.callstack):
            records = self.callstack[instruction['depth'] - 1]
            if len(records) < 2:
                return None
            mutator = SymbolicTaintAnalyzer.stack_taint_table[instruction['op']]
            values: list[BitVecRef] = []
    ...
    def set_tainted_record(self, record: TaintRecord, depth: int = -1, index: int = -1) -> None:
        self.callstack[depth][index] = record

    def get_tainted_record(self, depth: int = -1, index: int = -1) -> TaintRecord | None:
        try:
    ...
    @staticmethod
    def execute_instruction(record: TaintRecord, storage: dict[HexAddress | None, dict[str, list[BitVecRef] | Literal[False]]], instruction: TracedInstruction) -> TaintRecord:
    ...
    @staticmethod
    #@profile
    def mutate_stack_symbolically(record: TaintRecord, mutator: tuple[int, int], instruction: TracedInstruction) -> None:
    ...
    def mutate_stack(record: TaintRecord, mutator: tuple[int, int]) -> None:
        taint: list[BitVecRef] | Literal[False] = False
        for i in range(mutator[0]):
    ...
    @staticmethod
    def get_operand(record: TaintRecord, instruction: TracedInstruction, index: int) -> BitVecRef:
        if record.stack[-1]:
            assert record.stack[-index] != False
            return cast(BitVecRef, simplify(cast(list[BitVecRef], record.stack[-index])[0]))
        return BitVecVal(convert_stack_value_to_int(instruction['stack'][-index]), 256)

    @staticmethod
    def mutate_push(record: TaintRecord) -> None:
        SymbolicTaintAnalyzer.mutate_stack(record, (0, 1))

    @staticmethod
    def mutate_dup(record: TaintRecord, op: str) -> None:
        depth = int(op[3:])
        index = len(record.stack) - depth
        record.stack.append(record.stack[index])

    @staticmethod
    def mutate_swap(record: TaintRecord, op: str) -> None:
        depth = int(op[4:])
    ...
    @staticmethod
    def mutate_mload(record: TaintRecord, instruction: TracedInstruction) -> None:
        record.stack.pop()
    ...
    @staticmethod
    def mutate_mstore(record: TaintRecord, instruction: TracedInstruction) -> None:
        record.stack.pop()
    ...
    @staticmethod
    def mutate_sload(record: TaintRecord, storage: dict[HexAddress | None, dict[str, list[BitVecRef] | Literal[False]]], instruction: TracedInstruction) -> None:
        record.stack.pop()
        taint = False
    ...
    @staticmethod
    def mutate_sstore(record: TaintRecord, storage: dict[HexAddress | None, dict[str, list[BitVecRef] | Literal[False]]], instruction: TracedInstruction) -> None:
        record.stack.pop()
    ...
    @staticmethod
    def mutate_sha3(record: TaintRecord, instruction: TracedInstruction) -> None:
        record.stack.pop()
    ...
    @staticmethod
    def mutate_call_data_load(record: TaintRecord, instruction: TracedInstruction) -> None:
        value: list[BitVecRef] | Literal[False] = record.stack.pop()
        if record.input:
            index = convert_stack_value_to_hex(instruction['stack'][-1])
    ...
    @staticmethod
    def mutate_call_value(record: TaintRecord, instruction: TracedInstruction) -> None:
        record.stack.append(record.value)

    @staticmethod
    def mutate_copy(record: TaintRecord, op: str, instruction: TracedInstruction) -> None:
        if op == 'EXTCODECOPY':
            record.stack.pop()
    ...
    @staticmethod
    def mutate_create(record: TaintRecord, instruction: TracedInstruction) -> None:
        record.stack.pop()
        record.stack.pop()
        record.stack.pop()
        record.stack.append(False)
    ...
    @staticmethod
    def mutate_call(record: TaintRecord, op: str, instruction: TracedInstruction) -> None:
        record.stack.pop()
        record.stack.pop()
    ...
    @staticmethod
    def mutate_return_data_size(record: TaintRecord, op: str, instruction: TracedInstruction) -> None:
        record.stack.append(record.output)

    @staticmethod
    def extract_taint_from_memory(memory: dict[int, list[BitVecRef] | Literal[False]], offset: int, size: int) -> list[BitVecRef] | Literal[False]:
        taint: list[BitVecRef] | Literal[False] = []
        ...
        for j in range(len(keys)):
            ...
            if offset <= int(keys[j]):
                if memory[keys[j]]:
                    for k in cast(list[BitVecRef], memory[keys[j]]):
                        if not k in taint:
                            taint.append(k)
        if not taint:
            taint = False
    ...
    memory_access: dict[str, tuple[int, int]] = {
