...
from typing import TYPE_CHECKING, Callable, Type, Any, cast
...
def do_profile[**T_param, T_ret](filename: str, sortby: str = 'tottime') -> Callable[[Callable[T_param, T_ret]], Callable[T_param, T_ret]]:
    '''
    Constructor for function profiling decorator.
    '''
    def _do_profile(func: Callable[T_param, T_ret]) -> Callable[T_param, T_ret]:
        '''
        Function profiling decorator.
        '''
        @wraps(func)
        def profiled_func(*args: T_param.args, **kwargs: T_param.kwargs) -> T_ret:
            '''
            Decorated function.
            '''
...
class StatVar:
    ...
    def __get__(self, engine: EvolutionaryFuzzingEngine, cls: type[EvolutionaryFuzzingEngine]) -> Any | float:
        '''
        Getter.
        '''
        ...
        if stat_var is None:
            ...
            assert engine.ori_fitness is not None
    ...
    def __set__(self, engine: EvolutionaryFuzzingEngine, value: Any) -> None:
        '''
        Setter.
        '''
...
class EvolutionaryFuzzingEngine:
    ...
    def __init__(self, population: Population, selection: Selection, crossover: Crossover, mutation: Mutation, fitness: Callable[[Individual], float] | None = None, analysis: list[Type[OnTheFlyAnalysis]] | None = None, mapping: dict[str, str] | None = None) -> None:
        # Set logger.
    ...
    def run(self, ng: int) -> None:
        ...
        try:
            ...
            if self.fitness is None:
                raise AttributeError('No fitness function in EvolutionaryFuzzingEngine')
    ...
    def fitness_register(self, fn: Callable[[Individual], float]) -> None:
        '''
        A decorator for fitness function register.
        '''
        @wraps(fn)
        def _fn_with_fitness_check(indv: Individual) -> float:
            '''
        ...
        self.fitness = cast(Callable[[Individual], float], _fn_with_fitness_check)
        if self.ori_fitness is None:
            self.ori_fitness = cast(Callable[[Individual], float], _fn_with_fitness_check)
