...
class BasicBlock:
    def __init__(self) -> None:
        self.start_address: int | None = None
        self.end_address: int | None = None
    ...
    def __eq__(self, _other: BasicBlock) -> bool:
        return self.__dict__ == _other.__dict__

    def set_start_address(self, start_address) -> None:
    ...
    def set_end_address(self, end_address) -> None:
        self.end_address = end_address

    def get_end_address(self) -> int:
        assert self.end_address is not None
        return self.end_address

    def add_instruction(self, key: int, value: str) -> None:
...
class ControlFlowGraph:
    def __init__(self) -> None:
        ...
        self.visited_branches: dict[str, dict[int, VisitedBranchRecord]] = {}
        self.error_pcs: set[int] = set()
        self.can_send_ether: bool = False

    def build(self, bytecode: HexStr, evm_version: str) -> None:
        ...
        previous_push_value: int | None = None
        while current_pc < len(_bytecode):
            opcode = _bytecode[current_pc]
            ...
            if opcode == 86 or opcode == 87: # JUMP or JUMPI
                ...
                if opcode == 86 and previous_opcode and previous_opcode >= 96 and previous_opcode <= 127:
                    assert previous_push_value is not None
            ...
            if opcode >= 96 and opcode <= 127: # PUSH??
                size = opcode - 96 + 1
                _previous_push_value = ''
                for i in range(size):
                    try:
                        _previous_push_value += str(hex(_bytecode[current_pc+i+1])).replace('0x', '').zfill(2)
                    except Exception as e:
                        pass
                if _previous_push_value:
                    assert basic_block is not None
                    _previous_push_value = '0x' + _previous_push_value
                    basic_block.add_instruction(current_pc, self.opcode_to_mnemonic[evm_version][opcode]+' '+_previous_push_value)
                    previous_push_value = int(_previous_push_value, 16)
                    current_pc += size
    ...
    def execute(self, pc: int, stack: list[tuple[type, int | bytes]], mnemonic: str, visited_branches: dict[str, dict[int, VisitedBranchRecord]], error_pcs: set[int]) -> None:
        if mnemonic == 'JUMP':
    ...
    def save_control_flow_graph(self, filename: str, extension: str) -> None:
            ...
            if basic_block.get_end_address() in self.edges:
                ...
                if list(basic_block.get_instructions().values())[-1] == 'JUMPI':
                    if hex(basic_block.get_end_address()) in self.visited_branches and 0 in self.visited_branches[hex(basic_block.get_end_address())] and self.visited_branches[hex(basic_block.get_end_address())][0]['expression']:
                        f.write("'"+hex(basic_block.get_start_address())+"' -> '"+hex(self.edges[basic_block.get_end_address()][0])+"' [label=' "+str(cast(list['BoolRef'], self.visited_branches[hex(basic_block.get_end_address())][0]['expression'])[-1])+"',color='red'];\n")
                    else:
                    ...
                    if hex(basic_block.get_end_address()) in self.visited_branches and 1 in self.visited_branches[hex(basic_block.get_end_address())] and self.visited_branches[hex(basic_block.get_end_address())][1]['expression']:
                        f.write("'"+hex(basic_block.get_start_address())+"' -> '"+hex(self.edges[basic_block.get_end_address()][1])+"' [label=' "+str(cast(list['BoolRef'], self.visited_branches[hex(basic_block.get_end_address())][1]['expression'])[-1])+"',color='green'];\n")
                    else:
