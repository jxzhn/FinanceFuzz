...
from eth_utils.address import to_canonical_address, to_normalized_address

from z3 import BitVec, BitVecVal, BitVecNumRef, BoolRef, Not, Optimize, sat, simplify, is_expr
...
if TYPE_CHECKING:
    ...
    from engine.components import Population, Individual, Generator
    from engine.components.generator import FuzzTransactionInput
    from eth_typing import HexAddress, Address, HexStr

VisitedBranchRecord = TypedDict('VisitedBranchRecord', {
    'indv_hash': str,
    'chromosome': list['FuzzTransactionInput'],
    'transaction_index': int,
    'expression': list[BoolRef] | None
})
...
class ExecutionTraceAnalyzer(OnTheFlyAnalysis):
    ...
    def execute(self, population: Population, engine: EvolutionaryFuzzingEngine) -> None:
        ...
        executed_individuals: dict[str, Individual] = {}
    ...
    def register_step(self, g: int, population: Population, engine: EvolutionaryFuzzingEngine) -> None:
        ...
        assert self.env.execution_begin is not None
    ...
    def execution_function(self, indv: Individual, env: FuzzingEnvironment) -> None:
        ...
        for transaction_index, test in enumerate(indv.solution):
            ...
            _array_size_indexes: dict[int, int] = {}
            ...
            previous_branch_expression: BoolRef | None = None
            previous_branch_address: str | None = None
            previous_call_address: HexStr | None = None
            sha3: dict[int | bytes, bytes] = {}
            ...
            for i, instruction in enumerate(result.trace):
                ...
                if instruction['op'] == 'JUMPI':
                    ...
                    if tainted_record and tainted_record.stack and tainted_record.stack[-2]:
                        if jumpi_condition != 0:
                            previous_branch.append(cast(BoolRef, tainted_record.stack[-2][0] != 0))
                        else:
                            previous_branch.append(cast(BoolRef, tainted_record.stack[-2][0] != 0))
                ...
                # If something goes wrong, we need to clean some pools
                    ...
                    if previous_branch_expression is not None and is_expr(previous_branch_expression):
                        ...
                        previous_branch_expression_vars = list(get_vars(previous_branch_expression))
                        if len(previous_branch_expression_vars) == 1:
                            for var in previous_branch_expression_vars:
                                _str_var = str(var)
                                ...
                                if _str_var.startswith('calldataload_') or str(var).startswith('calldatacopy_'):
                                    ...
                                    assert _blocknumber is not None
                    ...
                    if input_index > 0 and _function_hash in env.interface:
                        ...
                        if input_index < len(env.interface[_function_hash]):
                            ...
                            if '[' in parameter_type:
                                ...
                                assert int(array_size_index) == array_size_index
    ...
    def get_coverage_with_children(self, children_code_coverage: dict[Address, set[int]], code_coverage: set[str]) -> int:
        code_coverage_num = len(code_coverage)

        for addr in children_code_coverage:
            code_coverage_num += len(children_code_coverage[addr])
        return code_coverage_num

    def symbolic_execution(self, indv_generator: Generator) -> None:
            ...
            if check == sat:
                ...
                for variable in model:
                    ...
                    if str(variable).startswith('balance'):
                        ...
                        if check == sat:
                            opt_model = opt.model()
                            ref = opt_model[variable]
                            assert isinstance(ref, BitVecNumRef)
                            balance = int(ref.as_long())
                        ...
                        ref = model[variable]
                        assert isinstance(ref, BitVecNumRef)
                        if ref.as_long() > 8 and ref.as_long() < 2**160:
                            account_address = normalize_32_byte_hex_address('0x'+hex(ref.as_long()).replace('0x', '').zfill(40))
                        ...
                        if '[' in indv_generator.interface[_function_hash][parameter_index]:
                            if indv_generator.interface[_function_hash][parameter_index].startswith('int'):
                                ref = model[variable]
                                assert isinstance(ref, BitVecNumRef)
                                argument = ref.as_signed_long()
                            elif indv_generator.interface[_function_hash][parameter_index].startswith('address'):
                                try:
                                    _function_hash = _d['chromosome'][transaction_index]['arguments'][0]
                                    ref = model[variable]
                                    assert isinstance(ref, BitVecNumRef)
                                    argument = normalize_32_byte_hex_address(hex(ref.as_long()))
                        ...
                        if '[' in indv_generator.interface[_function_hash][parameter_index]:
                            if indv_generator.interface[_function_hash][parameter_index].startswith('int'):
                                ...
                                # TOOD: Confuzzius miss this case
                                print(indv_generator.interface[_function_hash][parameter_index])
                            ...
                            try:
                                ...
                                assert isinstance(ref, BitVecNumRef)
                                argument = to_hex(
                                    force_bytes_to_address(int_to_big_endian(int(ref.as_long()))))
                                if not self.env.instrumented_evm.has_account(argument):
                                    self.env.instrumented_evm.restore_from_snapshot()
                                    self.env.instrumented_evm.accounts.append(self.env.instrumented_evm.create_fake_account(argument))
                                    self.env.instrumented_evm.create_snapshot()
                        ...
                        indv_generator.add_gaslimit_to_pool(_function_hash, ref.as_long())
                        ...
                        indv_generator.add_extcodesize_to_pool(_function_hash, _address, int(ref.as_long()))
                        ...
                        indv_generator.add_returndatasize_to_pool(_function_hash, _address, int(ref.as_long()))
